...
/**
 * The class defines a white piece
 */
public class WhitePawn extends Piece{
    ...
    /**
     * From the white piece current position gets ist valid coordinates
     * @return a list of valid moves
     */
    public List<Point> ableToMove(){
        List<Point> pieceList = new ArrayList<>();
        if (getX()+1 < getTABLE() && getY() + 1 < getTABLE()){
            pieceList.add(new Point(getX()+1,getY()+1));
        }
        if (getX()-1 >= 0 && getY() + 1 < getTABLE()){
            pieceList.add(new Point(getX()-1,getY()+1));
        }
